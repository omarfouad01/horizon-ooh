...
export default function AdminLogin() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setLoading(true)
    setTimeout(() => {
      if (login(email, pw)) navigate('/admin')
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-lg">
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            {error && <p className="text-xs text-red-500 font-semibold">Invalid credentials. Try again.</p>}
            <button type="submit" disabled={loading}
              className="w-full h-11 rounded-xl text-white font-bold text-sm uppercase tracking-wider disabled:opacity-50"
          ...
          <div className="mt-5 p-3 rounded-xl bg-gray-50 border border-gray-100">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <code className="text-xs font-mono font-bold text-gray-700 bg-white px-2 py-0.5 rounded border">admin@horizonooh.com</code>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs text-gray-500">Password</span>
                <code className="text-xs font-mono font-bold text-gray-700 bg-white px-2 py-0.5 rounded border">horizon2024</code>
              </div>
            </div>
          </div>
        </div>
