...
// Mirror the same preview-host exclusion used in apiStore.ts
const _PREVIEW_HOSTS_DS = ['skywork.website', 'skywork.ai', 'vercel.app', 'netlify.app', 'pages.dev', 'surge.sh', 'github.io'];
const _hostnameDS    = typeof window !== 'undefined' ? window.location.hostname : '';
const _isPreviewDS   = _PREVIEW_HOSTS_DS.some(h => _hostnameDS === h || _hostnameDS.endsWith('.' + h));
const _isLocalhostDS = _hostnameDS === 'localhost' || _hostnameDS === '127.0.0.1' ||
  _hostnameDS.startsWith('192.168.') || _hostnameDS.startsWith('10.');
const HAS_API_LOCAL = !!(_envApiUrl && _envApiUrl.trim() && _envApiUrl !== '/api') ||
  (!_isLocalhostDS && !_isPreviewDS);
...
export const simulatorTemplateStore = {
  add: async (data: Omit<SimulatorTemplate, 'id'> & { mockupFile?: File | null }) => {
    // Strip File object before local storage — store URL after API responds
    const { mockupFile, ...rest } = data as any;
    const item: SimulatorTemplate = { ...rest, id: uid() };
    if (HAS_API_LOCAL) {
      try {
        const apiPayload = mockupFile ? { ...rest, mockupFile } : rest;
        const res = await simulatorTemplatesApi.create(apiPayload);
        const saved = res.data?.data ?? res.data ?? item;
  ...
  update: async (id: string, data: Partial<SimulatorTemplate> & { mockupFile?: File | null }) => {
    const { mockupFile, ...rest } = data as any;
    if (HAS_API_LOCAL) {
      try {
        const apiPayload = mockupFile ? { ...rest, mockupFile } : rest;
        await simulatorTemplatesApi.update(id, apiPayload);
      } catch { /* ignore */ }
    }
    set(st => { const u = (st as any).simulatorTemplates.map((x: SimulatorTemplate) => x.id === id ? { ...x, ...rest } : x); lsSet(LS_TEMPLATES, u); return { simulatorTemplates: u }; });
  },
  remove: async (id: string) => {
