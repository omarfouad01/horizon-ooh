...
import { Plus, Pencil, Trash2, ShieldCheck, Eye, EyeOff, RefreshCw } from 'lucide-react'
...
// ─── User Form ────────────────────────────────────────────────────────────────
function UserForm({ editing, onClose }: { editing?: DashboardUser | null; onClose: () => void }) {
  ...
  function save(e: React.FormEvent) {
    ...
    if (isEdit && editing) {
      const updated = users.map(u => u.id === editing.id
        ? { ...u, name: f.name.trim(), email: f.email.trim(), role: f.role }
        : u
      )
      saveUsers(updated)
      // Update stored password hash if changed
      if (f.password) {
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="relative">
          <Field
            ...
            placeholder={isEdit ? 'Enter new password to change…' : 'At least 8 characters'}
          />
      ...
      <p className="text-[11px] text-gray-400 bg-amber-50 p-3 rounded-lg border border-amber-100">
        <strong>Note:</strong> In demo mode, credentials are saved locally. When connected to the real Laravel API, user management will use the backend authentication system.
      </p>

      <div className="flex gap-3 justify-end pt-3 border-t border-gray-100">
        <Btn variant="ghost" type="button" onClick={onClose}>Cancel</Btn>
        <Btn type="submit">{isEdit ? 'Save Changes' : 'Create User'}</Btn>
      </div>
    </form>
  )
}
...
export default function AdminDashboardUsers() {
  ...
  function handleDelete() {
    if (!del) return
    const updated = loadUsers().filter(u => u.id !== del.id)
    saveUsers(updated)
    setUsers(updated)
  ...
  function closeForm() { setForm(false); setEdit(null); refresh() }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            <button onClick={refresh} className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors">
              <RefreshCw size={14}/>
            </button>
      ...
      <div className="mb-6 p-4 rounded-xl flex items-start gap-3" style={{ background: '#F0F4FF', border: '1px solid #D6E0FF' }}>
        ...
        <div>
          ...
          <p className="text-[12px] text-blue-600 mt-0.5">
            ...
            In demo mode, credentials are stored locally. On your live server, connect this to your Laravel admin users endpoint.
          </p>
        </div>
      </div>
        ...
        <tbody>
          {users.length === 0 ? (
            ...
            <Tr key={u.id}>
              ...
              <Td className="text-[12px] text-gray-400">{u.createdAt}</Td>
              <Td>
                <div className="flex gap-1">
                  ...
                  <button className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors"
                    disabled={u.id === '1'}
                    title={u.id === '1' ? 'Cannot delete the primary admin' : 'Delete user'}
      ...
      {/* Form modal */}
      {form && (
        <Modal title={edit ? `Edit — ${edit.name}` : 'Add Dashboard User'} onClose={closeForm} wide>
